...
for j in range(1, len(paths)):
    ...
    for k in np.arange(len(sorted_dirs)):  # loop over directories
        ...
        streak_df = pd.DataFrame(
            {
                "Image": image,
                "Avg_Angle_StD": image_angle_std,
                "Avg_Major_Minor": image_major_minor,
                "Streak": streak_no_streak,
            }
        )
        ...
        print(
            ...
            counter,
            ", Number of images impacted by streaking =",
            streak_num,
        )
        ...
        if includestreaks == 0:
            ...
            f.write(
                "- Do size statistics include potential streak-impacted images? YES"
            )
        f.close()
    ...
    if vdist == 1:
        ...
        dstats.index = np.arange(
            1, len(dstats) + 1
        )  # changes the row index so that it starts at 1 instead of 0
