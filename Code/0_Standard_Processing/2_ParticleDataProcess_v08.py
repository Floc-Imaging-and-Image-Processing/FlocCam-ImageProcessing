...
v08: streak detection removed. v07 ran every image through a pre-trained
scikit-learn classifier (models/streak_remove_1.pickle) to flag images blurred
by high-speed movement during capture, and could drop those particles from the
size statistics. All of that is gone -- every image that passes the focus, edge,
and minimum-area filters now contributes to the PSD.
...
# --- time base for the grouped output ---

# 1_ImageProcess_v04.py writes 0_group_info.csv into the image directory, recording how
# much real time one group of images represents. These two values are only used as a
# fallback when that file is absent, e.g. for data grouped before it existed.
...
name_list = [
    ...
    "Round",
    "Solidity",
]  # works in the processing in imageJ_code_diffXX.txt
...
for j in range(1, len(paths)):
    ...
    dirs_1 = [
        ...
        if os.path.isdir(os.path.join(path_main, f))
    ...
    for k in np.arange(len(sorted_dirs)):  # loop over directories
        print(
            "Roaming through folder",
            int(k + 1),
            "of",
            len(sorted_dirs),
            "to find you the best flocs :)",
        )
        ...
        for f in np.arange(numfiles):  # loop over files in each directory
            ...
            if os.path.exists(datafile):
                ...
                temp.insert(
                    1,
                    "Img_no",
                    ((f + 1) * np.ones(len(temp))).astype(int).tolist(),
                    True,
                )
                col_2 = (
                    np.arange(total_particles, len(temp) + total_particles) + 1
                ).tolist()
                ...
                temp = temp[
                    (temp["MaxGreyValue"] >= focus)  # clarity crit
                    & (temp["BX"] + temp["Width"] < img_sz_x - edge_thickness)
                    & (temp["BX"] > edge_thickness)  # edge crit
                    & (temp["BY"] + temp["Height"] < img_sz_y - edge_thickness)
                    & (temp["BY"] > edge_thickness)  # edge crit
                    & (temp["Area"] >= minarea)
                ]  # min particle size in square pixels
    ...
    area_pixel.columns = np.arange(
        first, len(area_pixel.columns) + 1
    )  # rename the columns headers to match file names
    minor_axis.columns = np.arange(
    ...
    if darea == 1:
        d_mu = np.sqrt(
            4 * a_mu2 / np.pi
        )  # calculate the floc spherical diameter in pixels and put everything in microns
    else:
    ...
    if vdist == 1:
        ...
        for i in range(first, last):
            ...
            dvol_uL = (
                1e-9 * (np.pi / 6) * d**3
            )  # volume in microliters (1 micron^3 = 1e-9 microliters)

            values, base = np.histogram(dlog, bins=nb, weights=dvol_uL)

            cumulative = np.cumsum(
                values
            )  # adds up the frequencies (total is the total number of points)
            perc = cumulative / cumulative[len(cumulative) - 1]
            percfiner = perc
            percfiner = np.insert(percfiner, 0, 0)
            d16_mu[i] = np.exp(np.interp(0.16, percfiner, base))
            d50_mu[i] = np.exp(np.interp(0.5, percfiner, base))
            d84_mu[i] = np.exp(np.interp(0.84, percfiner, base))
            Sed_vol_uL[i] = np.sum(values)
        ...
        dstats.index = t_group  # elapsed time at the end of each group
        dstats.index.name = time_label

        dstats.to_csv(dstatsfile)

        # plot the distribution stats
        plt.subplot()
        plt.plot(t_group, d16_mu, "s-", label="$d_{16}$")
        plt.plot(t_group, d50_mu, "o-", label="$d_{50}$")
        plt.plot(t_group, d84_mu, "v-", label="$d_{84}$")
        # ylim(0,250)
        plt.xlabel("$t$ [" + group_time_unit + "]")
        plt.ylabel("$d_{f}$ [µm]")
        plt.legend()
        plt.title("by volume")
        plt.tight_layout()
        ...
        for i in np.arange(first, last):
            ...
            values = (
                values * (np.exp((base[1:] + base[:-1]) / 2)) ** w
            )  # include this to do the weighting
            cumulative = np.cumsum(
                values
            )  # adds up the frequencies (total is the total number of points)
            perc = cumulative / cumulative[len(cumulative) - 1]
        ...
        plt.tight_layout()
        plt.savefig(
            super_path + "/SizeStats-timeseries-d**" + str(w) + "-weighting.pdf"
        )
        plt.close()
