...
useLISST = 1
lisstbinsfile = "../LISST_bins/LISST_bins_sphere.csv"  # LISST_bins_random.csv

combine = "no"  # average over multiple output PSDs (groups). Choose "yes" and then set
# the first and last group of the averaging period below. Choose "no" to look at a single
# group of interest instead

groupfirst = 1
grouplast = 18

groupoi = 23  # individual distribution (group of interest). Groups are the numbered image
...
for j in range(1, len(paths)):
    ...
    data.to_csv(
        super_path + "/PSD_num" + dist + "_bintype_" + binmethod + ".csv", index=False
    )
